...
const adapter = new adapter_pg_1.PrismaPg({ connectionString: process.env.PG_URL_1 || process.env.DATABASE_URL });
...
async function main() {
    ...
    if (allAulas[0]) {
        ...
        if (!existsP)
            await prisma.progresso.create({ data: { moduloId: modulo.id, aulaId: allAulas[0].id, userId: atendente1.id, concluido: true } });
    }
    if (allAulas[1]) {
        const existsP = await prisma.progresso.findFirst({ where: { moduloId: modulo.id, aulaId: allAulas[1].id, userId: atendente1.id } });
        if (!existsP)
            await prisma.progresso.create({ data: { moduloId: modulo.id, aulaId: allAulas[1].id, userId: atendente1.id, concluido: true } });
    }
    if (allAulas[0]) {
        const existsP = await prisma.progresso.findFirst({ where: { moduloId: modulo.id, aulaId: allAulas[0].id, userId: atendente2.id } });
        if (!existsP)
            await prisma.progresso.create({ data: { moduloId: modulo.id, aulaId: allAulas[0].id, userId: atendente2.id, concluido: true } });
    }
    ...
    if (!existsN)
        await prisma.notification.create({ data: { fromId: admin.id, toId: atendente1.id, titulo: 'Bem-vindo!', mensagem: 'Sua conta foi criada na Academia PayGas!' } });
    // ============ ACTIVITY LOGS ============

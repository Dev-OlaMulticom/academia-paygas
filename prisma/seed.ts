...
async function main() {
	...
	const aulasData = [
		{
			...
			quiz: {
				...
				perguntas: [
					{
						pergunta: "Qual e a importancia da primeira impressao no atendimento?",
		...
		},
		{
			...
			quiz: {
				...
				perguntas: [
					{
						pergunta: "Qual e o primeiro passo ao atender um cliente insatisfeito?",
						opcaoA: "Discutir",
	...
	const modulo2Data = {
		...
		descricao:
			"Capacitacao em seguranca do trabalho, normas ABNT, procedimentos de emergencia e prevencao de acidentes em postos de combustivel.",
		ordem: 2,
	};
		...
		},
		{
			...
			quiz: {
				...
				perguntas: [
					...
					{
						pergunta: "A cada quanto tempo deve ser feita a auditoria de seguranca?",
					...
					{
						pergunta: "Qual certificacao e importante para postos de combustivel?",
						opcaoA: "ISO 14001",
						opcaoB: "NBR 15834",
	...
	if (allAulas[0]) {
		const existsP = await prisma.progresso.findFirst({
			where: { moduloId: modulo.id, aulaId: allAulas[0].id, userId: atendente1.id },
		});
		if (!existsP)
			await prisma.progresso.create({
				data: { moduloId: modulo.id, aulaId: allAulas[0].id, userId: atendente1.id, concluido: true },
			});
	}
	if (allAulas[1]) {
		const existsP = await prisma.progresso.findFirst({
			where: { moduloId: modulo.id, aulaId: allAulas[1].id, userId: atendente1.id },
		});
		if (!existsP)
			await prisma.progresso.create({
				data: { moduloId: modulo.id, aulaId: allAulas[1].id, userId: atendente1.id, concluido: true },
			});
	}
	if (allAulas[0]) {
		const existsP = await prisma.progresso.findFirst({
			where: { moduloId: modulo.id, aulaId: allAulas[0].id, userId: atendente2.id },
		});
		if (!existsP)
			await prisma.progresso.create({
				data: { moduloId: modulo.id, aulaId: allAulas[0].id, userId: atendente2.id, concluido: true },
			});
	}
	...
	if (!existingTrans1) {
		await prisma.pointsTransaction.create({
			data: { userId: atendente1.id, action: "LOGIN", points: 10, details: "Acesso a plataforma" },
		});
		await prisma.pointsTransaction.create({
			data: {
		...
		await prisma.pointsTransaction.create({
			data: { userId: atendente1.id, action: "QUIZ_PASS", points: 100, details: "Quiz aprovado com nota 10/10" },
		});
		await prisma.pointsTransaction.create({
			data: { userId: atendente1.id, action: "LESSON_COMPLETE", points: 50, details: "Aula: Comunicacao Eficaz" },
		});
		await prisma.pointsTransaction.create({
			data: { userId: atendente1.id, action: "QUIZ_PASS", points: 100, details: "Quiz aprovado com nota 7/10" },
		});
	}
	...
	if (!existingTrans2) {
		await prisma.pointsTransaction.create({
			data: { userId: atendente2.id, action: "LOGIN", points: 10, details: "Acesso a plataforma" },
		});
		await prisma.pointsTransaction.create({
			data: {
	...
	await prisma.activityLog.create({
		data: { userId: atendente1.id, acao: "Modulo Aberto", detalhes: "Excelencia no Atendimento" },
	});
	await prisma.activityLog.create({
		data: { userId: atendente1.id, acao: "Aula Concluida", detalhes: "Fundamentos do Atendimento" },
	});

...
import { PrismaClient } from "@packages/db/prisma/client";
...
async function testUsersFlow() {
	...
	try {
		...
		logResult(
			"READ User",
			foundUser ? "PASS" : "FAIL",
			foundUser ? `User found: ${foundUser.email}` : "User not found",
		);
...
async function testQuizFlow(aulaId: string | null) {
	...
	try {
		...
		logResult(
			...
			found && found.perguntas.length > 0 ? "PASS" : "FAIL",
			"Quiz with questions retrieved",
		);

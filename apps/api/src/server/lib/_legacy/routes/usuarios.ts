...
router.get("/", authenticate, authorize("ADMIN", "GESTOR"), async (req: AuthRequest, res) => {
	try {
		...
		const estIds = (users as any[])
			.map((u: any) => u.estabelecimentoId)
			.filter(Boolean);
...
router.put("/change-password", authenticate, async (req: AuthRequest, res) => {
	try {
		...
		await drizzleDb.update("user", { id: req.userId! }, { senha: hashedPassword });
...
router.get("/equipe/detalhe", authenticate, authorize("ADMIN", "GESTOR"), async (req: AuthRequest, res) => {
	try {
		const allUsersQuery =
			req.userRole === "GESTOR"
				? (drizzleDb.findMany("user", { where: { gestorId: req.userId } }) as Promise<any[]>)
...
router.post("/:userId/auto-approve", authenticate, authorize("ADMIN", "GESTOR"), async (req: AuthRequest, res) => {
	try {
		...
		if (tipo === "quiz" && targetId) {
			...
			if (!quizRow) return res.status(404).json({ error: "Quiz nao encontrado" });
		...
		if (tipo === "curso" && targetId) {
			...
			if (!cursoRow) return res.status(404).json({ error: "Curso nao encontrado" });

...
async function getApiSpec() {
	...
	return {
		openapi: "3.1.0",
		info: {
			title: "Academia PayGas API (Fastify Server)",
			description:
				"API REST para el sistema de aprendizaje Academia PayGas. Backend Fastify con Drizzle ORM y PostgreSQL.",
			version: "1.0.0",
		...
		endpoints: [
			// AUTH
			{
				...
				response: { token: "string (JWT)", user: { id: "string", email: "string", nome: "string", role: "string" } },
			},
			{
				method: "GET",
				path: "/api/auth/me",
			...
			},
			{
				...
				permission: "ADMIN, GESTOR",
				request_body: {
					required: ["email", "nome", "senha", "role"],
					properties: {
			...
			},
			{
				...
				tags: ["Usuarios"],
				permission: "ADMIN",
				request_body: { optional: ["nome", "email", "role", "gestorId"] },
			},
			{
				method: "DELETE",
				path: "/api/usuarios/:id",
			...
			{
				method: "GET",
				path: "/api/usuarios/equipe",
				summary: "Obtener equipo del gestor",
				tags: ["Usuarios"],
				permission: "ADMIN, GESTOR",
				response: {
					type: "array",
					items: { id: "string", nome: "string", email: "string", role: "string", xp: "number", certCount: "number" },
				},
			},
			...
			},
			{
				...
				request_body: {
					required: ["titulo"],
					optional: ["descricao", "ordem", "videoUrl", "videoInicio", "videoFim"],
					properties: {
						titulo: { type: "string" },
						descricao: { type: "string" },
						ordem: { type: "number" },
						videoUrl: { type: "string", format: "uri" },
						videoInicio: { type: "number", description: "Seconds" },
						videoFim: { type: "number", description: "Seconds" },
					},
				},
			},
			{
			...
			},
			{
				...
				request_body: {
					required: ["titulo"],
					optional: ["descricao", "videoUrl", "videoInicio", "videoFim", "duracaoMin"],
				},
			},
			{
				method: "PUT",
			...
			// CERTIFICATES
			{
				...
				tags: ["Certificates"],
				permission: "authenticated",
				description: "ADMIN ve todos, otros ven los suyos",
			},
			{
				method: "POST",
				path: "/api/certificates",
			...
			{
				method: "PUT",
				path: "/api/certificates/:id/approve",
			...
			{
				method: "PUT",
				path: "/api/certificates/:id/issue",
			...
			},
			{
				...
				tags: ["Notifications"],
				permission: "authenticated",
				request_body: { required: ["toId", "titulo", "mensagem"] },
			},
			{
				method: "PUT",
			...
			{
				method: "GET",
				path: "/api/progresso/stats",
				summary: "Estadisticas de progreso",
				tags: ["Progresso"],
				permission: "authenticated",
				response: {
					totalAulas: "number",
			...
			// HEALTH
			{
				...
				tags: ["System"],
				permission: "public",
				response: { status: "ok", encrypted: "boolean", timestamp: "datetime" },
			},
			...
			// DOCS
			{
				method: "GET",
				path: "/api/docs",
				summary: "Documentacion JSON de la API",
				tags: ["Docs"],
				permission: "public",
			},
			{
				method: "GET",
				path: "/api/docs/json",
				summary: "Spec JSON para agentes IA",
				tags: ["Docs"],
				permission: "public",
			},
			{
				method: "GET",
				path: "/api/docs/html",
				summary: "Documentacion HTML visual",
				tags: ["Docs"],
				permission: "public",
			},
		],
		data_models: {
			...
			Curso: {
				...
				videoInicio: "number|null (seconds)",
				videoFim: "number|null (seconds)",
				createdAt: "datetime",
				updatedAt: "datetime",
			},
			Aula: {
		...
		examples: {
			login: {
				...
				body: { email: "admin@academia.com", password: "admin123" },
				response: {
					token: "eyJhbGciOiJIUzI1NiIs...",
					user: { id: "clx...", email: "admin@academia.com", nome: "Admin", role: "ADMIN" },
				},
			},
			create_usuario: {
				request: "POST /api/usuarios",
				body: { email: "user@test.com", nome: "Juan Perez", senha: "pass123", role: "ATENDENTE" },
			},
		},
	};
}
...
/**
 * Docs routes — migrated from Express routes/docs.ts.
 * All endpoints are public (no auth required).
 */
const docsRoutes: FastifyPluginCallback = (fastify: FastifyInstance, _opts, done) => {
	// GET /api/docs/json
	fastify.get("/json", async (_request: FastifyRequest, reply: FastifyReply) => {
		const spec = await getApiSpec();
		reply.header("Content-Type", "application/json; charset=utf-8");
		reply.header("X-API-Version", "1.0.0");
		reply.header("X-Documentation-Type", "openapi-spec");
		return reply.send(spec);
	});

	// GET /api/docs/html
	fastify.get("/html", async (_request: FastifyRequest, reply: FastifyReply) => {
		const spec = await getApiSpec();
...
<title>Academia PayGas - Fastify API Docs</title>
<style>
...
<div class="container">
<header>
  <h1>Academia PayGas - Fastify API</h1>
  <p>Backend Fastify con Drizzle ORM + PostgreSQL. JWT Auth + AES-256-GCM Encryption.</p>
...
${endpointSections}
<div class="section" id="tag-Models"><h2>Data Models (Drizzle Schema)</h2>${modelsHtml}</div>
...
<footer>Academia PayGas Fastify API Documentation v1.0.0</footer>
</div></body></html>`;

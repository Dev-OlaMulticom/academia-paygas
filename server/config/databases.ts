...
class DatabaseRegistry {
  ...
  init() {
    ...
    if (this.entries.length === 0) {
      console.warn('[DB-REGISTRY] No PostgreSQL databases configured! Set PG_URL_1 or DATABASE_URL')
    } else {
      console.log(`[DB-REGISTRY] ${this.entries.length} database(s) registered: ${this.entries.map(e => e.name).join(', ')}`)
    }
  }
  ...
  private createEntry(name: string, url: string, priority: number): DatabaseEntry {
    ...
    try {
      ...
      console.error(`[DB-REGISTRY] Failed to create client for ${name}:`, error.message)
    }

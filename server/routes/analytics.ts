...
router.get('/overview', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    const [totalUsers, totalAulas, totalProgressos, totalCertificates, quizzesAprovados] = await Promise.all([
      prisma.user.count(),
      prisma.aula.count(),
...
router.get('/modules', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    console.error('[ANALYTICS MODULES ERROR]', error)
    res.status(500).json({ error: 'Erro ao buscar analytics de módulos' })
  }
})

// GET /api/analytics/personas
router.get('/personas', authenticate, async (req: AuthRequest, res) => {
  try {
...
router.get('/regions', authenticate, async (req: AuthRequest, res) => {
  try {
    ...
    console.error('[ANALYTICS REGIONS ERROR]', error)
    res.status(500).json({ error: 'Erro ao buscar regiões' })
  }
})

// GET /api/analytics/municipios
router.get('/municipios', authenticate, async (req: AuthRequest, res) => {
  try {
    const totalUsers = await prisma.user.count()

    const municipios = [
      { cidade: 'São Paulo, SP', postos: Math.round(totalUsers * 0.15), usuarios: Math.round(totalUsers * 0.2), pos: '🏆' },
      { cidade: 'Rio de Janeiro, RJ', postos: Math.round(totalUsers * 0.1), usuarios: Math.round(totalUsers * 0.13), pos: '🥈' },
      { cidade: 'Belo Horizonte, MG', postos: Math.round(totalUsers * 0.08), usuarios: Math.round(totalUsers * 0.1), pos: '🥉' },
      { cidade: 'Salvador, BA', postos: Math.round(totalUsers * 0.06), usuarios: Math.round(totalUsers * 0.08), pos: '4º' },
      { cidade: 'Fortaleza, CE', postos: Math.round(totalUsers * 0.05), usuarios: Math.round(totalUsers * 0.07), pos: '5º' },
      { cidade: 'Curitiba, PR', postos: Math.round(totalUsers * 0.05), usuarios: Math.round(totalUsers * 0.07), pos: '6º' },
    ]

    res.json(municipios)
  } catch (error) {
    console.error('[ANALYTICS MUNICIPIOS ERROR]', error)

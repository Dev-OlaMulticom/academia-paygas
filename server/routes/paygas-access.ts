  ...
  try {
    ...
    try {
      ...
      if (err instanceof PayGasApiError) {
        const status =
          err.code === 'PAYGAS_API_NOT_CONFIGURED'
            ? 503
            : err.code === 'PAYGAS_API_TIMEOUT'
              ? 504
              : 502
    ...
    if (isNewlyCreated && temporaryPassword) {
      sendPayGasAccessEmail(user.email, user.nome, temporaryPassword)
        .then(r => {
          if (!r.success) {
            console.warn(`[PAY-GAS] Falha ao enviar credenciais para ${user.email}: ${r.error}`)
          }
        })
        .catch(err => console.warn('[PAY-GAS] Erro no envio de credenciais:', err?.message))
    }
    ...
    console.error('[PAY-GAS ACCESS ERROR]', error)

...
async function syncDatabaseToTarget(
  ...
  console.log(`[DB-SYNC] Starting sync: ${source.name} → ${target.name}`)
  ...
  for (const table of TABLES_IN_ORDER) {
    ...
    if (sourceCount === -1 || targetCount === -1) {
      console.log(`[DB-SYNC]   ${table}: SKIP (table doesn't exist)`)
      continue
    }
    ...
    if (sourceCount === 0) {
      ...
      try {
        ...
        console.log(`[DB-SYNC]   ${table}: cleared (source empty)`)
      } catch { /* skip */ }
      continue
    }

    console.log(`[DB-SYNC]   ${table}: source=${sourceCount} target=${targetCount} — syncing...`)
    ...
    tablesSynced++
    console.log(`[DB-SYNC]   ${table}: synced ${sourceRows.length} rows`)
  }

  console.log(`[DB-SYNC] Complete: ${source.name} → ${target.name} (${tablesSynced} tables, ${totalRows} rows)`)
...
async function syncLoop() {
  ...
  const recovered = allDatabases.filter(e =>
    e.status === 'connected' && e.consecutiveFailures === 0 && e.lastCheck
  )
  ...
  for (const target of recovered) {
    ...
    if (targetUserCount === 0) {
      ...
      try {
        ...
        if (source) {
          console.log(`[DB-SYNC] Database ${target.name} is empty, syncing from ${source.name}`)
...
export function startSyncWorker() {
  if (syncInterval) {
    console.log('[DB-SYNC] Already running')
    return
  }

  console.log('[DB-SYNC] Starting background sync worker')
...
export async function forceSyncTo(targetName: string): Promise<boolean> {
  ...
  if (!target || !targetEntry) {
    console.error(`[DB-SYNC] Target database "${targetName}" not found`)
    return false
  }

  const source = await findBestSource(targetName)
  if (!source) {
    console.error(`[DB-SYNC] No healthy source database found for syncing to "${targetName}"`)
    return false
  }

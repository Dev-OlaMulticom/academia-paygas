import { decrypt, encrypt, initEncryptionKey } from "./crypto";
...
class ApiClient {
	...
	async sendNotificationBulk(params: {
		toId?: string;
		toRole?: string;
		toTeam?: boolean;
		titulo: string;
	...
	async getMyRolePermissions() {
		return this.request<{ role: string; label: string; description: string | null; permissions: any[] }>(
			"/role-permissions",
		);
	}
	...
	async updateRoleConfig(
		role: string,
		data: { permissions?: any[]; label?: string; description?: string; ativo?: boolean; ordem?: number },
	) {

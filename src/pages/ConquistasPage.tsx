			...
			<div className="trophy-grid">
				{conquistas.length === 0 ? (
					<div className="empty-state">
						...
						<p>
							{canManage ? "Nenhuma conquista configurada. Crie a primeira!" : "Nenhuma conquista disponivel ainda."}
						</p>
					</div>
				) : (
					conquistas.map((c) => (
						...
						>
								...
								<div className="conq-progress">
									...
									<div className="conq-progress-bar">
										<div
											...
											style={{ width: `${Math.min(c.progresso || 0, 100)}%`, background: c.cor }}
										/>
									</div>
								</div>
							)}
							...
							{canManage && (
								<div className="conq-card-actions">
									...
									<button
										className={`btn-secondary conq-btn-toggle ${c.ativo ? "inactive" : "active"}`}
										onClick={() => toggleAtivo(c)}
									>
										{c.ativo ? (
											<>
												<i className="icon-pause icon-xs" /> Desativar
											</>
										) : (
											<>
												<i className="icon-play icon-xs" /> Ativar
											</>
										)}
									</button>
									{isAdmin && (
						...
						<div className="form-field">
							...
							<div className="conq-icon-grid">
								{ICONES.map((icon) => (
									<button
										key={icon}
										className={`conq-icon-btn ${form.icone === icon ? "selected" : ""}`}
						...
						<div className="form-field">
							...
							<div className="conq-color-grid">
								{CORES.map((color) => (
									<button
										key={color}
										className={`conq-color-btn ${form.cor === color ? "selected" : ""}`}

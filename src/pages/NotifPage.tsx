	...
	const handleSend = async () => {
		...
		try {
			if (sendTarget === "all" && isAdmin) {
				...
				await api.sendNotificationBulk({
					toRole: newNotif.toRole,
		...
		<div className="page active">
			<div className="page-header">
				...
				<div className="notif-header-actions">
					<button className="btn-secondary notif-mark-read-btn" onClick={handleMarkAllRead}>
						<i className="icon-check icon-sm" /> Marcar todas como lidas
					</button>
					{(isAdmin || isGestor) && (
			...
			<div className="notif-list">
				{notifs.map((notif) => (
					<div key={notif.id} className={`notif-item ${!notif.lida ? "unread" : ""}`}>
						<div
							className="notif-icon notif-icon-box"
							style={{ background: !notif.lida ? "var(--pg-orange-lt)" : "var(--gray-100)" }}
						>
				...
				<div className="notif-modal-overlay">
					<div className="notif-modal">
						...
						<div className="form-field">
							...
							<div className="notif-target-btns">
								{isAdmin && (
									<>
										<button
											className={`btn-secondary notif-target-btn ${sendTarget === "all" ? "active" : ""}`}
											onClick={() => setSendTarget("all")}
										>
											Todos os Usuarios
										</button>
										<button
											className={`btn-secondary notif-target-btn ${sendTarget === "role" ? "active" : ""}`}
								...
								{isGestor && (
									<button
										className={`btn-secondary notif-target-btn ${sendTarget === "team" ? "active" : ""}`}
								...
								<button
									className={`btn-secondary notif-target-btn ${sendTarget === "user" ? "active" : ""}`}
						...
						{sendTarget === "role" && isAdmin && newNotif.toRole && (
							<div className="notif-info-box blue">
								A mensagem sera enviada para todos os usuarios com perfil <b>{newNotif.toRole}</b>.
							</div>
						)}
						{sendTarget === "team" && isGestor && (
							<div className="notif-info-box green">
								A mensagem sera enviada para todos os atendentes da sua equipe ({getTeamMembers().length} membro(s)).
							</div>
						)}
						...
						<div className="form-field">
							...
							<textarea
								className="form-input"
								value={newNotif.mensagem}
								onChange={(e) => setNewNotif({ ...newNotif, mensagem: e.target.value })}
								rows={4}

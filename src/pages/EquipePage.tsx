...
export function EquipePage({ user }: EquipePageProps) {
  ...
  useEffect(() => { if (!loading) loadDetail() }, [loading, loadDetail])

  const handleAutoApprove = async (userId: string, tipo: 'quiz' | 'aula' | 'modulo', targetId: string, label: string) => {
            ...
            <div key={mod.id} className="eq-detail-mod">
              <div className="eq-detail-mod-header" onClick={() => toggleModulo(modKey)}>
                <div className="eq-detail-mod-info">
                  ...
                  <span className="eq-detail-mod-pct" style={{ color: pct === 100 ? 'var(--pg-green)' : 'var(--gray-500)' }}>
                    {mod.aulasConcluidas}/{mod.totalAulas} aulas ({pct}%)
                  </span>
                  {mod.quizzesTotal > 0 && (
                    <span className="eq-detail-mod-quiz-count" style={{ color: mod.quizzesAprovados === mod.quizzesTotal ? 'var(--pg-green)' : 'var(--gray-500)' }}>
                  ...
                  {mod.aulas?.map((aula: any) => {
                    ...
                    return (
                      <div key={aula.id} className={`eq-detail-aula ${aula.concluido ? 'completed' : ''}`}>
                        <div className="eq-detail-aula-header" onClick={() => quiz ? toggleAula(aulaKey) : undefined}>
                          <i className={aula.concluido ? 'icon-check-circle icon-xs' : 'icon-circle icon-xs'} style={{ color: aula.concluido ? 'var(--pg-green)' : 'var(--gray-300)', flexShrink: 0 }} />
                            ...
                            <div className="eq-detail-aula-quiz">
                              {qr ? (
                                <span className={`eq-quiz-badge ${qr.concluido ? 'approved' : 'failed'}`}>
                                  {qr.concluido ? <><i className="icon-check icon-xs" /> {qr.nota}/{qr.total}</> : <><i className="icon-x icon-xs" /> {qr.nota}/{qr.total}</>}
                                </span>
                              ) : (
                              ...
                              {(isAdmin || isGestor) && !qr?.concluido && (
                                <button
                                  ...
                                  onClick={(e) => { e.stopPropagation(); handleAutoApprove(memberId, 'quiz', quiz.id, `Quiz "${aula.titulo}"`) }}
                                >
                                  {approving === `quiz-${quiz.id}` ? <i className="icon-loader icon-xs" /> : <i className="icon-check icon-xs" />}
                                </button>
                              )}
                              {quiz && <i className={`icon-chevron-${isAulaExpanded ? 'up' : 'down'} icon-xs`} style={{ color: 'var(--gray-400)' }} />}
                            </div>
                          )}
                          {!quiz && (isAdmin || isGestor) && !aula.concluido && (
                            <button
                              ...
                              onClick={(e) => { e.stopPropagation(); handleAutoApprove(memberId, 'aula', aula.id, `Aula "${aula.titulo}"`) }}
                            >
                              {approving === `aula-${aula.id}` ? <i className="icon-loader icon-xs" /> : <i className="icon-check icon-xs" />}
                            </button>
                          )}
                        </div>
                        ...
                        {isAulaExpanded && quiz && (
                          <div className="eq-detail-quiz-body">
                            <div className="eq-detail-quiz-meta">
                              Nota minima: {quiz.notaMinima}/10 | {quiz.totalPerguntas} {pluralize(quiz.totalPerguntas, 'pergunta')}
                            </div>
                            ...
                            {qr && qr.respostas && (
                              <div className="eq-detail-quiz-answers">
                                {quiz.perguntas?.map((pergunta: any, idx: number) => {
                                  ...
                                  return (
                                    <div key={pergunta.id} className={`eq-detail-answer ${isCorrect ? 'correct' : 'wrong'}`}>
                                      <span className="eq-detail-answer-icon">{isCorrect ? '✓' : '✗'}</span>
                                      <div className="eq-detail-answer-content">
                                        <span className="eq-detail-answer-q">{idx + 1}. {pergunta.pergunta}</span>
                                        <span className="eq-detail-answer-opts">
                                          <span className="eq-detail-answer-user">Resp: <b>{userAnswer || '-'}</b></span>
                                          {!isCorrect && <span className="eq-detail-answer-correct">Correta: <b>{pergunta.correta}</b></span>}
                                        </span>
                                      </div>
                                    </div>
                                  )
  ...
  const renderAtendenteRow = (member: any, i: number) => {
    ...
    const completedMods = detail?.modulos?.filter((m: any) => m.aulasConcluidas === m.totalAulas && m.totalAulas > 0).length || 0
          ...
          <div className="eq-user-stats">
            ...
            <div className="eq-user-stat eq-user-stat-progress">
              <div className="progress-mini">
                <div className={`progress-mini-fill ${(member.progress || 0) === 100 ? 'done' : ''}`} style={{ width: (member.progress || 0) + '%' }} />
              </div>
            ...
            {detail && (
              <>
                ...
                {totalQuizzes > 0 && (
                  <div className="eq-user-stat">
                    <span className={`eq-quiz-stat ${passedQuizzes === totalQuizzes && totalQuizzes > 0 ? 'done' : ''}`}>
            ...
            <span className={`status-pill ${(member.certCount || 0) > 0 ? 'pill-green' : 'pill-gray'}`}>{(member.certCount || 0) > 0 ? <><i className="icon-check icon-xs" />{member.certCount}</> : 'Pendente'}</span>
            <span className={`status-pill ${member.ativo !== false ? 'pill-green' : 'pill-gray'}`}>{member.ativo !== false ? 'Ativo' : 'Inativo'}</span>
  ...
  if (isGestor) {
    ...
    return (
      <div className="page active">
        ...
        {members.length > 0 ? (
          <div className="eq-users-list">
            {members.map(renderAtendenteRow)}
          </div>
        ) : (
        ...
        teams.map((team: any, idx: number) => (
          <div key={team.gestor?.id || idx} className="eq-team-section">
            ...
            {team.membros?.length > 0 ? (
              <div className="eq-users-list">
                {team.membros.map(renderAtendenteRow)}
              </div>
            ) : (

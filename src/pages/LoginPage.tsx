...
export function LoginPage({ onLogin }: LoginPageProps) {
  ...
  async function handlePayGasAccess() {
    ...
    if (!trimmed) {
      setPayGasError(
        payGasType === "cpf" ? "Informe seu CPF." : "Informe seu e-mail.",
      );
      return;
    }
    if (payGasType === "cpf") {
  ...
  const renderForgotForm = () => (
    <>
      ...
      <p className="login-desc">
        Informe o email da sua conta. Voce recebera um codigo de 6 digitos para
        redefinir sua senha.
      </p>
      {resetError && <div className="login-alert error">{resetError}</div>}
      {resetSuccess && (
        <div className="login-alert success">{resetSuccess}</div>
      )}
      <div className="field">
        <label>E-mail</label>
        <input
  ...
  const renderCodeSentForm = () => (
    <>
      ...
      <p className="login-desc">
        Um codigo de 6 digitos foi enviado para <strong>{resetEmail}</strong>. O
  ...
  const renderResetForm = () => (
    <>
      ...
      <p className="login-desc">
        Codigo verificado! Agora defina sua nova senha.
      </p>
      {resetError && <div className="login-alert error">{resetError}</div>}
      {resetSuccess && (
        <div className="login-alert success">{resetSuccess}</div>
    ...
    <div className="modal-overlay" onClick={closePayGasModal}>
      ...
      >
        <button
          type="button"
          className="login-back-btn"
          onClick={closePayGasModal}
        >
          <i className="icon-arrow-left icon-sm" /> Voltar ao login
        </button>
        <h2 className="login-heading">Acesso PayGas</h2>
        <p className="login-desc">
          Entre com seu CPF ou e-mail cadastrado no sistema PayGas para acessar
          a Academia.
        </p>

        <div
          className="paygas-type-toggle"
        ...
        {payGasSuccess && (
          <div className="login-alert success">{payGasSuccess}</div>
        )}
        ...
        <div className="field">
          ...
          <input
            ...
            placeholder={
              payGasType === "cpf" ? "000.000.000-00" : "seu@email.com"
            }
    ...
    <div id="screen-login">
      <div className="login-panel">
        ...
        {view === "login" && (
          <>
            ...
            <p>
              Acesse sua conta para continuar aprendendo e crescendo no
              ecossistema PayGas.
            </p>
          </>
        )}
      ...
      <div className="login-bg">
        ...
        <div className="login-bg-content">
          ...
          <p>
            A Academia PayGas conecta postos, parceiros e comunidades em todo o
            Brasil com conteudo profissional, modulos personalizados e
            certificacao reconhecida.
          </p>
          <div className="login-stats login-stats-grid">
            <div className="login-stat">
              <b>{publicStats?.alunos?.toLocaleString("pt-BR") || "—"}</b>
              <span>Usuarios ativos</span>
            </div>
            <div className="login-stat">
              <b>{publicStats?.notas || "—"}</b>
              <span>Modulos disponiveis</span>
            </div>
            <div className="login-stat">
              <b>{publicStats?.horas || "—"}</b>
              <span>Horas de conteudo</span>
            </div>
            <div className="login-stat">
              <b>{publicStats?.certificados || "—"}</b>

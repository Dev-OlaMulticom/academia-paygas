		...
		<div>
			...
			<div className="cards-grid nat-grid">
				{[
					{
						icon: "👥",
						val: totalUsers.toLocaleString("pt-BR"),
					...
					{
						icon: "⭐",
						val: overview ? `${Math.min(5, 3 + (overview.completionRate || 0) / 30).toFixed(1)}` : "—",
						label: "NPS Médio Brasil",
						color: "#FEF0E6",
						trend: "↑ baseado em conclusão",
					},
				].map((c, i) => (
					...
					{regions.map((r, i) => (
						<div key={i} className="nat-region-row">
							<div className="nat-region-header">
								...
								<span className="nat-region-stats">
									{r.users.toLocaleString("pt-BR")} · <b className="nat-region-growth">{r.growth}</b>
								</span>
							</div>
					...
					{modules.length === 0 ? (
						...
						modules.slice(0, 5).map((m, i) => (
							<div key={i} className="nat-region-row">
								...
								<div className="nat-bar">
									<div className="nat-bar-fill" style={{ width: `${m.conclusao}%`, background: "var(--pg-blue)" }}>
										{m.conclusao}%
									</div>
								</div>
							</div>
				...
				<div className="table-wrap">
					<table>
						...
						<tbody>
							{municipios.length === 0 ? (
								<tr>
									<td colSpan={4} className="nat-municipio-empty">

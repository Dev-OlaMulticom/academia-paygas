...
type PointsAction = 'LOGIN' | 'MODULE_OPEN' | 'LESSON_COMPLETE' | 'MODULE_COMPLETE' | 'QUIZ_CORRECT' | 'QUIZ_PASS' | 'CERTIFICATE'
...
export async function awardPoints(userId: string, action: PointsAction, details?: string): Promise<number> {
  ...
  await prisma.$transaction([
    prisma.pointsTransaction.create({ data: { userId, action, points, details } }),
    prisma.user.update({ where: { id: userId }, data: { xp: { increment: points } } }),
  ])
  return points
}
...
export async function getTeamPoints(gestorId?: string) {
  ...
  return {
    users: users.map((u: any, i: number) => ({ ...u, rank: i + 1, level: Math.floor(u.xp / 2000) + 1 })),
    totalXp,
